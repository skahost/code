...
show_dashboard() {
    ...
    echo -e "${C}╭──────────────────────────────────────────────────────────────╮${NC}"
    echo -e "${C}│${NC} ${W}⚡ SKA HOST VIRTUAL MACHINE ENGINE ${P}(KVM ACCELERATED)${NC}       ${C}│${NC}"
    echo -e "${C}├──────────────────────────────────────────────────────────────┤${NC}"
    ...
    if lsof "$img_file" 2>/dev/null | grep -q qemu-system; then
        ...
        if [[ -n "$pid" ]]; then
            if ps -p "$pid" -o cmd= | grep -q "$vm_name"; then
                read -p "$(echo -e "  ${Y}╰─➤${NC} Kill existing process & restart? (y/N): ")" kill_choice
...
edit_vm_config() {
    ...
    if load_vm_config "$vm_name"; then
        ...
        while true; do
            ...
            read -p "$(echo -e "  ${C}╰─➤${NC} Select option: ")" edit_choice
            
            case $edit_choice in
                1) read -p "$(prompt_input "New hostname: ")" HOSTNAME ;;
                2) read -p "$(prompt_input "New username: ")" USERNAME ;;
                3) read -rs -p "$(prompt_input "New password: ")" PASSWORD; echo ;;
                4) read -p "$(prompt_input "New SSH port: ")" SSH_PORT ;;
                5) read -p "$(prompt_input "GUI Mode (true/false): ")" GUI_MODE ;;
                6) read -p "$(prompt_input "Port Forwards: ")" PORT_FORWARDS ;;
                7) read -p "$(prompt_input "Memory (MB): ")" MEMORY ;;
                8) read -p "$(prompt_input "CPUs: ")" CPUS ;;
                9) read -p "$(prompt_input "Disk Size: ")" DISK_SIZE ;;
                0) return 0 ;;
...
resize_vm_disk() {
    ...
    if load_vm_config "$vm_name"; then
        ...
        read -p "$(echo -e "  ${C}╰─➤${NC} Enter new disk size (e.g., 50G): ")" new_disk_size
...
show_vm_performance() {
    ...
    if load_vm_config "$vm_name"; then
        if is_vm_running "$vm_name"; then
            ...
            if [[ -n "$qemu_pid" ]]; then
                echo -e "\n  ${G}╭─[ 📈 QEMU STATS ]${NC}"
                ps -p "$qemu_pid" -o pid,%cpu,%mem,sz,rss,vsz | awk '{print "  │  " $0}'
                echo -e "  ${G}╰──────────────────────${NC}"
...
fix_vm_issues() {
    ...
    if load_vm_config "$vm_name"; then
        echo -e "\n  ${Y}╭─[ 🔧 FIX OPTIONS ]${NC}"
        echo -e "  ${Y}│${NC}  [1] Remove lock files"
        echo -e "  ${Y}│${NC}  [2] Recreate seed iso"
        echo -e "  ${Y}│${NC}  [3] Recreate Config"
        echo -e "  ${Y}│${NC}  [4] Kill stuck processes"
        echo -e "  ${Y}╰──────────────────────${NC}"
        
        read -p "$(echo -e "  ${Y}╰─➤${NC} Select Fix: ")" fix_choice
        case $fix_choice in
...
while true; do
    ...
    echo -e "  ${B}│${NC}  ${DG}[${Y}5${DG}]${NC} ${P}✏️  Edit VM Config${NC}"
    echo -e "  ${B}│${NC}  ${DG}[${Y}6${DG}]${NC} ${R}🗑️  Delete VM${NC}"
    echo -e "  ${B}│${NC}  ${DG}[${Y}7${DG}]${NC} ${B}📈 Resize VM Disk${NC}"
    echo -e "  ${B}│${NC}  ${DG}[${Y}8${DG}]${NC} ${W}📉 Show VM Performance${NC}"
    echo -e "  ${B}│${NC}  ${DG}[${Y}9${DG}]${NC} ${Y}🔧 Fix VM Issues${NC}"
    echo -e "  ${B}│${NC}"
    echo -e "  ${B}│${NC}  ${DG}[${R}0${DG}]${NC} ${DG}❌ Exit System${NC}"
    echo -e "  ${B}╰────────────────────────────────────────${NC}"
    echo ""
    
    read -p "$(echo -e "  ${B}╰─➤${NC} Select Task (0-9): ")" choice
    ...
    case $choice in
        ...
        5) 
            if [ $vm_count -gt 0 ]; then
                read -p "$(echo -e "  ${P}╰─➤${NC} Enter VM number to EDIT: ")" vm_num
