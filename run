#!/bin/bash
# ================================================
# Ultra-Rich Premium Menu Script - FIXED VERSION
# Line endings must be LF (not CRLF) - Save as LF in VS Code
# ================================================
...
# Safe bash options (pipefail included)
set -euo pipefail
...
if [[ "$choice" != "1" && "$choice" != "2" ]]; then
    echo -e "\n ${C_RED}❌ Invalid option. Exiting...${NC}"
    exit 1
fi

# ==========================================
# LOADING + FINAL EXECUTION
# ==========================================
clear
echo ""
...
if [[ "$choice" == "1" ]]; then
    ...
    for i in {3..1}; do
        echo -ne "\r ${C_GLOW}Launching Interface in ${i}...${NC} "
        sleep 1
    done
    echo -e "\r ${C_GREEN}>>> SYSTEM ONLINE <<< ${NC}\n"

    # Fixed execution for Option 1
    echo -e "${C_CYAN}Launching Hosting Tool...${NC}"
    curl -sL https://raw.githubusercontent.com/sdgamer8263-sketch/SDGAMER.HOST/main/ty.sh | bash
    ...
    # Fixed execution for Option 2 (old link was 404)
    echo -e "${C_RED}Launching Hacking Tool...${NC}"
    echo -e "${C_YELLOW}Note: Old link was not found. If you have new link, replace it.${NC}"
    # Agar naya link hai toh yahan daal do, warna comment out rakh sakte ho
    # curl -sL "YOUR_NEW_LINK_HERE" | bash
fi
